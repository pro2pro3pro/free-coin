...
import { db, upsertUser, getUser, setNormalCoin, addNormalCoin, setVipCoin, addVipCoin, insertClaimGenerated, getDailyLink, setDailyLink, getClaimBySubid, markClaimAwarded, countAwardedTodayByPlatform, hasAwardedOnIP, sumCoinsForUserBetween, resetAllNormalCoins, setMeta, getMeta } from "./src/db.js";
...
// Register commands script (used on start)
async function registerCommands(){
  const commands = [
    new SlashCommandBuilder().setName("getcoin").setDescription("Nhận link vượt coin (menu 5 phút)"),
    new SlashCommandBuilder().setName("checkcoin").setDescription("Xem Normal/VIP coin, tổng tuần/tháng và hệ số")
  ].map(c => c.toJSON());
  const rest = new REST({ version: "10" }).setToken(process.env.DISCORD_TOKEN);
  try {
    await rest.put(Routes.applicationCommands(process.env.DISCORD_CLIENT_ID), { body: commands });
    console.log("✔ Registered slash commands");
  } catch(e){ console.error("Register commands error:", e.message); }
}

// Utilities
function todayStr(){ return dayjs().format("YYYY-MM-DD"); }
function makeSubId(userId, platform){ return `${platform}-${userId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`; }
...
async function ensureUserDailyLink(userId, platform){
  ...
  const longUrlBase = process.env.BASE_CLAIM_URL || `http://localhost:${process.env.PORT||3000}/claim`;
  const longUrl = `${longUrlBase}?platform=${platform}&subid=${encodeURIComponent(subid)}&uid=${userId}`;
...
function buildMenuEmbed(userId){
  ...
  return new EmbedBuilder()
    ...
    .setDescription(`Hệ số hiện tại: x${m.toFixed(3)}\\nNormal coin reset vào Thứ 2 hàng tuần.`)
    .addFields(
    ...
    ).setColor(0x5865F2).setFooter({ text:"Menu sống 5 phút - ai bấm cũng được, mỗi người link riêng" });
}
...
function menuButtons(disabled=false){
  return new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("getcoin:help").setLabel("Xem coin").setStyle(ButtonStyle.Secondary).setDisabled(disabled)
  );
}
...
async function handleGetcoinClick(interaction, platform){
  ...
  if ((counts[platform] || 0) >= (PLATFORM_LIMITS[platform] || 0)) {
    const embed = new EmbedBuilder().setTitle("Hết lượt hôm nay").setDescription(`Bạn đã dùng hết lượt cho ${platform}`).setColor(0xED4245);
  ...
  try {
    ...
    await dm.send({ embeds:[ new EmbedBuilder().setTitle(`Link ${platform} của bạn`).setDescription(`Coin gốc: ${base}\\nHệ số: x${multiplier.toFixed(3)}\\nNếu claim bây giờ: ~ ${total} coin\\n\\nLink của bạn:\\n${entry.link}`).setColor(0x2ECC71) ]});
  } catch {}
  try { await interaction.reply({ content:"Mình đã gửi link vào DM của bạn nhé!", ephemeral:true }); } catch {}
  // try update public menu message to show remaining counts (best-effort)
  try { await interaction.message.edit({ embeds:[buildMenuEmbed(userId)], components:[menuButtons(false)] }); } catch {}
}
...
async function handleCheckcoin(interaction, ephemeral=true){
  ...
  // week/month range
  const now = dayjs();
  const weekStart = now.day() === 0 ? now.add(-6,"day").startOf("day") : now.add(1-now.day(),"day").startOf("day");
  const weekEnd = dayjs(weekStart).add(6,"day");
  const weekly = sumCoinsForUserBetween(userId, weekStart.format("YYYY-MM-DD"), weekEnd.format("YYYY-MM-DD"));
  const monthStart = now.startOf("month"), monthEnd = now.endOf("month");
  const monthly = sumCoinsForUserBetween(userId, monthStart.format("YYYY-MM-DD"), monthEnd.format("YYYY-MM-DD"));
...
client.on("interactionCreate", async(interaction)=>{
  ...
  } catch (e) { console.error("Interaction error", e); try{ if (!interaction.replied) await interaction.reply({ content:"Lỗi, thử lại", ephemeral:true}); }catch{} }
});
...
async function registerOnBoot(){
  ...
  try { await rest.put(Routes.applicationCommands(process.env.DISCORD_CLIENT_ID), { body: commands }); console.log("Commands registered"); } catch(e){ console.error("reg cmds", e.message); }
}
...
app.get("/admin", (req,res) => {
  ...
  const adminList = (process.env.ADMIN_WHITELIST_IDS||"").split(",").map(s=>s.trim()).filter(Boolean);
...
app.get("/claim", (req,res) => {
  ...
  if (hasAwardedOnIP(ip, date, platform)) return res.status(429).send("IP này đã claim hôm nay cho platform này.");
  const counts = countAwardedTodayByPlatform(uid, date);
  if ((counts[platform]||0) >= (PLATFORM_LIMITS[platform]||0)) return res.status(429).send("Hết lượt hôm nay.");
  if (claim.status !== "awarded") {
    ...
    if (process.env.DISCORD_WEBHOOK_URL) {
      axios.post(process.env.DISCORD_WEBHOOK_URL, { content: `✅ <@${uid}> nhận ${total} coin từ ${platform} (IP: ${ip})` }).catch(()=>{});
    }
    // DM user
    try { client.users.fetch(uid).then(u=>u.send(`Bạn vừa nhận ${total} coin từ ${platform}`)).catch(()=>{}); } catch {}
  }
...
app.get("/admin/logs", (req,res) => {
  const q = { user_id: req.query.user_id || "", platform: req.query.platform || "", status: req.query.status || "", from:req.query.from||"", to:req.query.to||"" };
...
setInterval(()=>{
  ...
  } catch(e){ console.error("weekly reset err", e); }
}, 60*1000);